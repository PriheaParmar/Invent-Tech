...
def _get_prefetched_related(instance, related_name):
    cache = getattr(instance, "_prefetched_objects_cache", None) or {}
    return cache.get(related_name)


def _sum_prefetched_values(objects, field_name):
    total = Decimal("0")
    for obj in objects:
        total += getattr(obj, field_name, None) or Decimal("0")
    return total


def _sum_prefetched_nested_values(instance, related_name, nested_name, field_name):
    related_objects = _get_prefetched_related(instance, related_name)
    if related_objects is None:
        return None

    total = Decimal("0")
    for related_obj in related_objects:
        nested_objects = _get_prefetched_related(related_obj, nested_name)
        if nested_objects is None:
            return None
        total += _sum_prefetched_values(nested_objects, field_name)
    return total
...
class ERPNotification(OwnedModel):
    PRIORITY_CHOICES = [
        ("high", "High"),
        ("medium", "Medium"),
        ("low", "Low"),
    ...
    kind = models.CharField(max_length=30, choices=KIND_CHOICES, default="info")
...
class Material(models.Model):
    ...
    material_code = models.CharField(max_length=30, blank=True, default="")
    ...
    composition = models.CharField(max_length=160, blank=True, default="")
    ...
    class Meta:
        ...
        unique_together = [
            ("owner", "material_code"),
            ("owner", "material_kind", "name"),
        ]

    def __str__(self):
        code = f"{self.material_code} - " if self.material_code else ""
...
class Party(models.Model):
    ...
    party_code = models.CharField(max_length=20, blank=True, default="")
    party_name = models.CharField(max_length=150)
    party_category = models.CharField(max_length=20, choices=PARTY_CATEGORY_CHOICES, default="general")
    ...
    def __str__(self):
        code = f"{self.party_code} - " if self.party_code else ""
        return f"{code}{self.party_name}"
...
class Firm(models.Model):
    ...
    def default_shipping_address(self):
        if hasattr(self, "addresses"):
            default_address = self.addresses.filter(is_default=True).order_by("id").first()
            if default_address:
                return default_address
            return self.addresses.order_by("id").first()
        return None

    @property
    def full_address(self):
        default_address = self.default_shipping_address
        if default_address:
...
class FirmAddress(models.Model):
    firm = models.ForeignKey("Firm", on_delete=models.CASCADE, related_name="addresses")
    ...
    def __str__(self):
        title = self.label or self.firm.firm_name
        return f"{self.firm.firm_name} - {title}"
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            self.firm.addresses.exclude(pk=self.pk).filter(is_default=True).update(is_default=False)
...
class Vendor(OwnedModel):
    VENDOR_TYPE_CHOICES = [
        ("yarn", "Yarn Supplier"),
        ("greige", "Greige Supplier"),
        ("ready", "Ready Supplier"),
        ("trims", "Trims Supplier"),
        ("general", "General Supplier"),
    ]

    vendor_code = models.CharField(max_length=20, blank=True, default="")
    name = models.CharField(max_length=180)
    vendor_type = models.CharField(max_length=20, choices=VENDOR_TYPE_CHOICES, default="general")
    contact_person = models.CharField(max_length=120, blank=True, default="")
...
class YarnPurchaseOrder(OwnedModel):
    ...
    po_number = models.CharField(max_length=30, blank=True, default="")  # Reference No.
    ...
    def total_inward_qty(self):
        prefetched_total = _sum_prefetched_nested_values(self, "inwards", "items", "quantity")
...
class YarnPurchaseOrderItem(models.Model):
    ...
    def inward_qty_total(self):
        prefetched_items = _get_prefetched_related(self, "inward_items")
...
class YarnPOInwardItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if (self.quantity or Decimal("0")) <= 0 and (self.received_qty or Decimal("0")) > 0:
...
class DyeingPurchaseOrderItem(models.Model):
    ...
    def accepted_inward_qty_total(self):
        prefetched_items = _get_prefetched_related(self, "inward_items")
        if prefetched_items is not None:
            return _sum_prefetched_values(prefetched_items, "accepted_qty")
    ...
    def rejected_inward_qty_total(self):
        prefetched_items = _get_prefetched_related(self, "inward_items")
        if prefetched_items is not None:
            return _sum_prefetched_values(prefetched_items, "rejected_qty")
    ...
    def hold_inward_qty_total(self):
        prefetched_items = _get_prefetched_related(self, "inward_items")
        if prefetched_items is not None:
            return _sum_prefetched_values(prefetched_items, "hold_qty")
...
class ReadyPurchaseOrder(OwnedModel):
    ...
    reviewed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="reviewed_ready_purchase_orders",
    )
    reviewed_at = models.DateTimeField(null=True, blank=True)
...
class ReadyPOInwardItem(models.Model):
    ...
    # keep quantity for compatibility with current code
    quantity = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    ...
    def save(self, *args, **kwargs):
        ...
        total_split = (
            (self.accepted_qty or Decimal("0"))
            + (self.rejected_qty or Decimal("0"))
            + (self.hold_qty or Decimal("0"))
        )
...
class BOM(OwnedModel):
    ...
    def _sum_related_amount(self, related_name, field_name):
        related_manager = getattr(self, related_name, None)
        if related_manager is None:
            return Decimal("0")
        return related_manager.aggregate(total=Sum(field_name)).get("total") or Decimal("0")

    @property
    def material_total(self):
        return self._sum_related_amount("material_items", "cost")

    @property
    def accessory_total(self):
        return self._sum_related_amount("accessory_items", "cost")

    @property
    def expense_total(self):
        return self._sum_related_amount("expense_items", "price")

    @property
    def jobber_type_process_total(self):
        return self._sum_related_amount("jobber_type_processes", "price")

    @property
    def jobber_detail_total(self):
        return self._sum_related_amount("jobber_details", "price")

    @property
    def process_total(self):
        detail_total = self.jobber_detail_total
        if detail_total > 0:
            return detail_total
        return self.jobber_type_process_total

    @property
    def base_cost_total(self):
        return (
            (self.price or Decimal("0"))
            + (self.tie_dye_price or Decimal("0"))
            + self.material_total
    ...
    def damage_amount(self):
        ...
        return (self.base_cost_total * damage_percent / Decimal("100")).quantize(Decimal("0.01"))

    def recalculate_final_price(self, save=True):
        self.final_price = (self.base_cost_total + self.damage_amount).quantize(Decimal("0.01"))
        if save and self.pk:
...
class ProgramStartJobber(models.Model):
    ...
    assigned_qty = models.DecimalField(max_digits=12, decimal_places=2, default=0)
...
class ProgramJobberChallan(OwnedModel):
    ...
    last_inward_type = models.ForeignKey(
        "InwardType",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="program_jobber_challans",
    )
    last_inward_date = models.DateField(null=True, blank=True)
    last_inward_time = models.TimeField(null=True, blank=True)
    last_inward_remarks = models.TextField(blank=True, default="")
...
class DispatchChallanSize(models.Model):
    ...
    def __str__(self):
        return f"{self.dispatch_challan.challan_no} - {self.size_name}"
...
class QRCodeRecord(OwnedModel):
    ...
    def generate_qr_image(self, payload):
        qr = qrcode.QRCode(
            version=1,
            box_size=10,
            border=4,
...
class QRCodeScanLog(OwnedModel):
    qr_record = models.ForeignKey("QRCodeRecord", on_delete=models.CASCADE, related_name="scan_logs")
    ...
    scanned_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="qr_code_scans_done",
    )
...
class MaintenanceExpenseItem(models.Model):
    ...
    def __str__(self):
        expense_name = self.expense.name if self.expense_id else "Expense"
        return f"{self.maintenance.month_key} - {expense_name}"
...
class MaintenanceRecord(OwnedModel):
    ...
    def recalculate_totals(self):
        expense_total = (
            self.expense_items.aggregate(total=Sum("price")).get("total")
            or Decimal("0")
        )
        self.expense_total = Decimal(expense_total).quantize(Decimal("0.01"))
        self.cost_total = (
            Decimal(self.inward_total or 0) + Decimal(self.expense_total or 0)
        ).quantize(Decimal("0.01"))
    
    def save(self, *args, **kwargs):
        self.expense_total = Decimal(self.expense_total or 0).quantize(Decimal("0.01"))
        self.cost_total = (
            Decimal(self.inward_total or 0) + Decimal(self.expense_total or 0)
        ).quantize(Decimal("0.01"))
